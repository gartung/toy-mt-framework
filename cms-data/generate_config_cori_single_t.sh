...
for ((x=4;x<=64;x+=4)); do
   ...
   echo "#SBATCH -N1        " >>srun-driver-${x}.sh
   echo "#SBATCH -c64       " >>srun-driver-${x}.sh
   ...
   for ((y=1;y<=${x};y+=1));do
    echo /global/homes/g/gartung/build/BuildProducts/bin/TBBDemo /global/homes/g/gartung/toy-mt-framework/cms-data/reco_hipileup_5_2_0_sleeping_perfectIO.${nSimulEvtP}s.${nThreadsP}t.${iterations}i.config\& >>srun-driver-$x.sh
    done
