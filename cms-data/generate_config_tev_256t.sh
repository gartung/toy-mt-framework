...
for ((x=256;x>=16;x-=16)); do
    ...
    echo $nThreadsP
    iterations=$stepsize*$x
    echo "qsub -q knl -l nodes=1:knl,walltime=12:00:00 -A CMS-MT-Tests ~/toy-mt-framework/cms-data/driver-busywait-${nSimulEvtP}s-${nThreadsP}t.sh">>driver-busywait-256t.sh
